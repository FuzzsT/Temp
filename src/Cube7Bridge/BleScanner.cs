...
public sealed class BleScanner
{
    ...
    public async Task RunAsync(CancellationToken ct)
    {
        ...
        watcher.Received += (_, e) =>
        {
            ...
            if (_seen.TryAdd(e.BluetoothAddress, name))
            {
                ...
                Console.WriteLine($"[ble] {FormatAddress(e.BluetoothAddress)} RSSI={e.RawSignalStrengthInDBm} name='{name}' services=[{uuids}]");
                Log(new { type = "advertisement", timeUtc = DateTime.UtcNow, address = FormatAddress(e.BluetoothAddress), addressRaw = e.BluetoothAddress, rssi = e.RawSignalStrengthInDBm, name, serviceUuids = e.Advertisement.ServiceUuids.Select(x => x.ToString()).ToArray(), manufacturer = e.Advertisement.ManufacturerData.Select(x => new { companyId = x.CompanyId, data = ToHex(x.Data) }).ToArray(), dataSections = e.Advertisement.DataSections.Select(x => new { dataType = x.DataType, data = ToHex(x.Data) }).ToArray() });
            }
            bool matchAddr = _cfg.Address.HasValue && _cfg.Address.Value == e.BluetoothAddress;
            bool matchName = _cfg.NameContains.Any(x => !string.IsNullOrWhiteSpace(x) && name.Contains(x, StringComparison.OrdinalIgnoreCase));
            if (matchAddr || matchName) tcs.TrySetResult(e.BluetoothAddress);
        };

        watcher.Start();
        Console.WriteLine("[ble] active scan started; no writes are performed.");
    ...
    {
        Console.WriteLine($"[ble] connecting {FormatAddress(address)}...");
        ...
        foreach (var service in servicesResult.Services)
        {
            ...
            foreach (var ch in charsResult.Characteristics)
            {
                ...
                if (props.HasFlag(GattCharacteristicProperties.Read))
                {
                    try
                    {
                        ...
                        if (rr.Status == GattCommunicationStatus.Success)
                            { var hex = ToHex(rr.Value); Console.WriteLine($"[gatt]     READ {hex}"); Log(new { type = "read", timeUtc = DateTime.UtcNow, service = service.Uuid, uuid = ch.Uuid, value = hex }); }
                        else
                ...
                {
                    ch.ValueChanged += (_, e) => { var hex = ToHex(e.CharacteristicValue); Console.WriteLine($"[gatt]     NOTIFY {ch.Uuid} {hex}"); Log(new { type = "notify", timeUtc = DateTime.UtcNow, service = service.Uuid, uuid = ch.Uuid, value = hex }); };
